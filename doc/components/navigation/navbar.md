...
VelocityNavbar(
  title: '标题',
  centerTitle: true,
  leading: IconButton(icon: VelocityIcon(Icons.arrow_back), onPressed: () {}),
  actions: [
    IconButton(icon: VelocityIcon(Icons.share), onPressed: () {}),
  ],
)
```
...
| 属性         | 类型                           | 默认值 | 说明              |
| ------------ | ------------------------------ | ------ | ----------------- |
| items        | List<VelocityBottomNavbarItem> | -      | 导航项列表 (必填) |
