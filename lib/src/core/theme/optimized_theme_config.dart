...
import 'package:zephyr_ui/zephyr_ui.dart' hide ZephyrThemeConfig;
...
class ZephyrOptimizedThemeConfig {
  ...
  ZephyrThemeData _createTheme(
    ...
    switch (variant) {
      case 'enterprise':
        return ZephyrThemeConfig.createEnterpriseTheme(brightness: brightness);
      case 'modern':
        return ZephyrThemeConfig.createModernTheme(brightness: brightness);
      case 'nature':
        return ZephyrThemeConfig.createNatureTheme(brightness: brightness);
      default:
  ...
  /// 生成缓存键
  String _generateCacheKey(Brightness brightness, String? variant,
      Map<String, dynamic>? customColors) {
  ...
  void warmUpCache() {
    ...
    final themes = [
      ZephyrOptimizedThemeData.light(),
      ZephyrOptimizedThemeData.dark(),
      ZephyrThemeConfig.createEnterpriseTheme(brightness: Brightness.light),
      ZephyrThemeConfig.createEnterpriseTheme(brightness: Brightness.dark),
      ZephyrThemeConfig.createModernTheme(brightness: Brightness.light),
      ZephyrThemeConfig.createModernTheme(brightness: Brightness.dark),
      ZephyrThemeConfig.createNatureTheme(brightness: Brightness.light),
      ZephyrThemeConfig.createNatureTheme(brightness: Brightness.dark),
    ];
    ...
    if (foundation.kDebugMode) {
      debugPrint('🔥 Theme cache warmed up with ${themes.length} themes');
    }
  }
  ...
  void clearCache() {
    ...
    if (foundation.kDebugMode) {
      debugPrint('🧹 Theme cache cleared');
    }
  }
  ...
  Map<String, dynamic> getPerformanceReport() {
    ...
    return {
      ...
      'memory': {
        ...
        'averageThemeSizeKB': _themeCache.isNotEmpty
            ? (memoryUsage * 1024 / _themeCache.length).toStringAsFixed(2)
            : '0',
      },
  ...
  double _estimateMemoryUsage() {
    // 简化的内存估算
    const baseThemeSize = 0.5; // MB per theme
  ...
  List<String> _getOptimizationRecommendations() {
    ...
    if (stats['cacheSize'] > 20) {
      recommendations.add(
          '⚠️ Theme cache is large (${stats['cacheSize']} items). Consider reducing cache size.');
    }

    if (double.parse(stats['hitRate']) < 0.7) {
      recommendations.add(
          '⚠️ Cache hit rate is low (${stats['hitRate']}). Consider optimizing cache strategy.');
    }

    if (!stats['isWarmedUp']) {
      recommendations
          .add('💡 Consider warming up cache during app initialization.');
    }
  ...
  void optimizeCache() {
    ...
    if (foundation.kDebugMode && keysToRemove.isNotEmpty) {
      debugPrint('🗑️ Removed ${keysToRemove.length} unused themes from cache');
    }
  }
}
...
class ZephyrThemeConfigBuilder {
  ...
  String? _variant;
  final Map<String, dynamic> _customColors = {};
  bool _useCache = true;
