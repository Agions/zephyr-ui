...
/// 轮播图指示器类型
enum chromaCarouselIndicatorType {
  dots,
  numbers,
  fraction,
  none,
...
/// 轮播图动画类型
enum chromaCarouselAnimationType {
  slide,
  fade,
  scale,
  depth,
...
/// 轮播图组件
class chromaCarousel extends StatefulWidget {
  final List<chromaCarouselItem> items;
  final double height;
  final bool autoPlay;
  final Duration autoPlayInterval;
  final bool enableInfiniteScroll;
  final bool showIndicators;
  final chromaCarouselIndicatorType indicatorType;
  final chromaCarouselAnimationType animationType;
  ...
  final chromaCarouselTheme? theme;

  const chromaCarousel({
    Key? key,
    required this.items,
    this.height = 200.0,
    this.autoPlay = true,
    ...
    this.indicatorType = chromaCarouselIndicatorType.dots,
    this.animationType = chromaCarouselAnimationType.slide,
  ...
  @override
  State<chromaCarousel> createState() => _chromaCarouselState();
}

class _chromaCarouselState extends State<chromaCarousel> {
  ...
  Widget build(BuildContext context) {
    final effectiveTheme = chromaCarouselTheme.resolve(context, widget.theme);

    return chromaPerformanceWidget(
      widgetName: 'chromaCarousel',
      child: MouseRegion(
  ...
  Widget _buildCarouselContent() {
    if (widget.items.isEmpty) {
      return const Center(
        child: chromaText.body(text: '没有可显示的内容'),
      );
    }
  ...
  Widget _buildCarouselItem(chromaCarouselItem item, int index) {
  ...
  Matrix4 _buildTransform(double value) {
    switch (widget.animationType) {
      case chromaCarouselAnimationType.slide:
        return Matrix4.translationValues(value * 100, 0, 0);
      case chromaCarouselAnimationType.fade:
        return Matrix4.identity();
      case chromaCarouselAnimationType.scale:
        final scale = 1.0 - (value * 0.2);
        return Matrix4.diagonal3Values(scale, scale, 1.0);
      case chromaCarouselAnimationType.depth:
  ...
  Widget _buildControls(chromaCarouselTheme effectiveTheme) {
    return Positioned(
      top: 0,
      bottom: 0,
      left: 0,
  ...
  Widget _buildControlButton({
    ...
    required chromaCarouselTheme effectiveTheme,
  }) {
    return Align(
      alignment: alignment,
      child: Container(
  ...
  Widget _buildIndicators(chromaCarouselTheme effectiveTheme) {
    ...
    return Positioned(
      ...
      child: Center(
        child: switch (widget.indicatorType) {
          chromaCarouselIndicatorType.dots => _buildDotsIndicator(effectiveTheme),
          chromaCarouselIndicatorType.numbers => _buildNumbersIndicator(effectiveTheme),
          chromaCarouselIndicatorType.fraction => _buildFractionIndicator(effectiveTheme),
          chromaCarouselIndicatorType.none => const SizedBox.shrink(),
        },
      ),
    );
  }

  Widget _buildDotsIndicator(chromaCarouselTheme effectiveTheme) {
    return Row(
      mainAxisSize: MainAxisSize.min,
  ...
  Widget _buildNumbersIndicator(chromaCarouselTheme effectiveTheme) {
    return Row(
      mainAxisSize: MainAxisSize.min,
      children: List.generate(widget.items.length, (index) {
        final isActive = index == _realPageIndex;
  ...
  Widget _buildFractionIndicator(chromaCarouselTheme effectiveTheme) {
    return Container(
...
/// 轮播图主题
class chromaCarouselTheme extends ThemeExtension<chromaCarouselTheme> {
  ...
  @override
  chromaCarouselTheme lerp(ThemeExtension<chromaCarouselTheme>? other, double t) {
    if (other is! chromaCarouselTheme) {
      return this;
    }
    return chromaCarouselTheme(
  ...
  static chromaCarouselTheme of(BuildContext context) {
    final theme = Theme.of(context).extension<chromaCarouselTheme>();
    return theme ?? _createDefaultTheme(chromaThemeData.of(context));
  }

  static chromaCarouselTheme resolve(
    BuildContext context,
    chromaCarouselTheme? theme,
  ) {
    return theme ?? of(context);
  }

  static chromaCarouselTheme _createDefaultTheme(chromaThemeData chromaTheme) {
    return chromaCarouselTheme(
